...
export function collectEvents(data, { name, color, timezone, windowStart, windowEnd }) {
  ...
  for (const key of Object.keys(data)) {
    ...
    // node-ical marks zoned datetimes with a `tz` prop (a TZID, or 'Etc/UTC'
    // for trailing-Z values) — those Dates are correct absolute instants.
    // Floating times (no TZID, no Z) are parsed with `new Date(y, m, ...)` in
    // the *server's* local zone, so their absolute value shifts with the host
    // TZ (e.g. +2h in a UTC container for a CEST wall clock). Recover the
    // original wall clock via local formatting and re-anchor it in the display
    // timezone. rrule occurrences and Dates derived from a floating DTSTART
    // inherit the same server-local basis, so the event's own start decides.
    const toZoned = (when, sourceTz) =>
      sourceTz ? dayjs(when).tz(timezone) : dayjs.tz(dayjs(when).format('YYYY-MM-DDTHH:mm:ss'), timezone)

    const push = (when, summaryOverride, sourceTz) => {
      const d = toZoned(when, sourceTz)
      events.push({
    ...
    if (ev.rrule) {
      ...
      for (const occ of rule.between(windowStart.toDate(), windowEnd.toDate(), true)) {
        ...
        push(occ, undefined, ev.start?.tz)
      }
      // node-ical registers each override under BOTH a date-only key and a
      // full-ISO key (same object) — dedupe or every edited occurrence doubles.
      for (const rec of new Set(Object.values(overrides))) {
        if (rec.status === 'CANCELLED') continue
        const recStart = toZoned(rec.start, rec.start?.tz)
        if ((recStart.isAfter(windowStart) || recStart.isSame(windowStart)) && recStart.isBefore(windowEnd)) {
          ...
          push(rec.start, recSummary, rec.start?.tz)
        }
      }
    } else {
      const start = toZoned(ev.start, ev.start?.tz)
      if ((start.isAfter(windowStart) || start.isSame(windowStart)) && start.isBefore(windowEnd)) {
        push(ev.start, undefined, ev.start?.tz)
      }
    }
  }
  return events
