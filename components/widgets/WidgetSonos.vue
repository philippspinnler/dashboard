<template>
  <div>
    <h1 class="title">Sonos</h1>
    <div v-if="!currentlyPlaying">
      <p class="subtitle">Zurzeit wird nichts abgespielt</p>
    </div>
    <div v-if="currentlyPlaying" class="sonos-container">
      <div>
        <h2>{{ truncatedName }}</h2>
        <p>{{ detail }}</p>
      </div>
      <div v-if="image" class="artwork">
        <img :src="image" alt="Album artwork" />
      </div>
    </div>
  </div>
</template>

<script setup>
const { data } = useWidgetData('/api/sonos', 10000)

const currentlyPlaying = computed(() => data.value?.playing || false)
const name = computed(() => data.value?.artist || '')
const detail = computed(() => data.value?.song || '')
const image = computed(() => data.value?.image || '')

const truncatedName = computed(() =>
  name.value.length > 20 ? name.value.slice(0, 20) + '...' : name.value,
)
</script>

<style scoped>
.sonos-container {
  display: flex;
}

.sonos-container .artwork > img {
  height: 3.52rem;
  width: 3.52rem;
}

.sonos-container .artwork {
  margin-left: 1.1rem;
}
</style>
